...
import com.developtech.efuelfo.ui.adapters.vehicleOwner.SelectVehicleAdapter;
...
public class FilterVehicleDialogFragment extends DialogFragment{
    ...
    void init()
    {
        if (getContext()!=null)
        {
            tvTitle.setText(getContext().getResources().getString(R.string.filterbyvehicleno));
            tvSelectName.setText(getContext().getResources().getString(R.string.selectVehicles));
        ...
        {
            vehicleList.add(model.getVehicle());
        }

        selectVehicleAdapter = new SelectVehicleAdapter(vehicleList);
        LinearLayoutManager layoutManager = new LinearLayoutManager(getContext(), LinearLayoutManager.VERTICAL, false);
        rvDialog.setLayoutManager(layoutManager);
        rvDialog.setAdapter(selectVehicleAdapter);
    ...
    void filter()
    {
        ...
        List<AllVehicleResponseModel> addedList = selectVehicleAdapter.getSelectedList();

        if(addedList.size()==0)
        {
            showMsg(rootLayout, context.getResources().getString(R.string.please_select_vehicle));
            return;
        }
        ...
        for (PaymentHistoryResponseModel model : historyModelList)
        {
            ...
            {
                for (AllVehicleResponseModel vehicleModel : addedList)
                {
                    if(model.getVehicle().getVehicleNumber().equals(vehicleModel.getVehicleNumber()))
                    {
        ...
        selectVehicleAdapter.resetSelections();
    }
