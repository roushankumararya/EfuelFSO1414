...
import com.developtech.efuelfo.ui.adapters.vehicleOwner.SelectStationAdapter;
...
public class FilterStationDialogFragment extends DialogFragment
{
    ...
    List<CreditAgreementResponseModel> historyModelList;
    SelectStationAdapter selectStationAdapter;
    ...
    public void setData(Context context, List<CreditAgreementResponseModel> historyModelList, FilterUpdateListener listener)
    {
    ...
    void init()
    {
        ...
        if (getContext()!=null)
        {
            tvTitle.setText(getContext().getResources().getString(R.string.filter_by_station));
            tvSelectName.setText(getContext().getResources().getString(R.string.selectStation));
            tvSelectDate.setText(getContext().getResources().getString(R.string.select_date));
        }

        stationList = new ArrayList<>();

        for(CreditAgreementResponseModel model : historyModelList)
        {
//            stationList.add(model.getFuelStation());
        }

        selectStationAdapter = new SelectStationAdapter(stationList);
        LinearLayoutManager layoutManager = new LinearLayoutManager(getContext(), LinearLayoutManager.VERTICAL, false);
        rvDialog.setLayoutManager(layoutManager);
        rvDialog.setAdapter(selectStationAdapter);
    ...
    List<CreditAgreementResponseModel> filteredList = new ArrayList<>();
    ...
    void filter()
    {
        ...
        List<FuelStationResponseModel> addedList = selectStationAdapter.getSelectedList();

        if(addedList.size()==0)
        {
            showMsg(rootLayout, context.getResources().getString(R.string.please_select_fuelstation));
            return;
        }
        ...
        for (CreditAgreementResponseModel model : historyModelList)
        {
            ...
            {
                for (FuelStationResponseModel stationModel : addedList)
                {
//                    if(model.getFuelStation().getId().equals(stationModel.getId()))
//                    {
...
//                    }
        ...
        listener.updateData(filteredList);

        dismiss();

        selectStationAdapter.resetSelections();
    }
