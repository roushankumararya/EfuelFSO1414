package com.developtech.Scanner;
...
public class ViewFinderView extends View {
    ...
    private static final float PORTRAIT_WIDTH_RATIO = 7f / 10;// default 7f / 8
...
//		paint.setColor(resources.getColor(R.color.viewfinder_laser));
        ...
        // canvas.drawRect(mFramingRect.left + 2, middle - 1, mFramingRect.right
        // - 1, middle + 2, paint);
        canvas.drawRect(mFramingRect.left + 2, p, mFramingRect.right - 1,
                p + 5, paint);

        System.out.println(p + " " + mFramingRect.bottom + " "
                + (mFramingRect.top + 10) + " " + (p <= mFramingRect.bottom)
                + " " + (p >= mFramingRect.bottom));

        if (mode == 0) {
            p += 10;
            if (p >= mFramingRect.bottom) {
                mode = 1;
            }
        }
        if (mode == 1) {
            p -= 10;
            if (p <= mFramingRect.top) {
                mode = 0;
            }
        ...
        } else if (p <= mFramingRect.bottom) {

        }

        postInvalidateDelayed(80L, mFramingRect.left - POINT_SIZE,
    ...
    public synchronized void updateFramingRect() {
        ...
        if (topOffset < 1) {
            //topOffset = (viewResolution.y - height) / 2;
        topOffset = (viewResolution.y - width) / 2;
        } else {
            // topOffset=50;
        }

        // int = 0;//(viewResolution.y - height) / 2;
